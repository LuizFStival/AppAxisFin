...
begin
  ...
  loop
    ...
    if signed_amount = 0 then
      raise exception 'A fatura contém um lançamento com valor inválido.';
    end if;
    ...
    if coalesce((item->>'is_projected')::boolean, false) then
      ...
      values (
        ...
        null,
        nullif(item->>'notes', ''),
        coalesce((item->>'is_reimbursable')::boolean, false),
  ...
  set balance = balance - calculated_amount
  where id = p_account_id and user_id = current_user_id
